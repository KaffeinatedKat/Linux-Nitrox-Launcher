...
cp -r ~/.steam/steam/steamapps/compatdata/264710 ~/.steam/steam/steamapps/compatdata/NitroxLauncher || failed=1

if [[ $failed == 1 ]]; then
  echo -e "\e[31mPlease install Subnautica in the deafult steam directory and run it at least once\e[0m" && exit 1
fi

export STEAM_COMPAT_DATA_PATH=~/.steam/steam/steamapps/compatdata/NitroxLauncher
